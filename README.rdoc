...
* Try to catch socket setup that'll have the context ripped out by the GC eg. ZMQ::Context.new.socket(:PUSH)
...
* ZMQ::Pollable implementation
* ZmqCastPollable() to replace ZmqAssertSocket + ZmqAssertSocketNotPending() on poller and loop registration and removal
  - rbczmq_get_pollitem(pollitem); rbczmq_get_pollitem(socket); rbczmq_get_pollitem(VALUE)
  - struct { VALUE socket; VALUE io; zmq_pollitem pollitem } rbczmq_pollitem;
* Do not clobber local scope from macros
