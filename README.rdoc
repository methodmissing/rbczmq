...
* Go through the full network stack on localhost (TCP/IP transport)
* The sender and receiver endpoints are Ruby processes which coerce transferred data to native String objects
  on both ends.
* There's just a definite method dispatch cost in addition to intermittent pauses from the Garbage Collector
* It's still plenty fast for most soft real-time (a language with automatic memory management cannot easily comply to hard real-time guarantees anyways) applications and we're able to push in excess of 1 gigabit/s with the 1024 byte payloads.
